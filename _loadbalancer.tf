locals {
  ...
  create_target_groups_tmp = [
    for instance_name, instance_config in var.ec2_instance_parameters :
    [
      for port_key, port_values in try(instance_config.ports, {}) :
      [
  ...
  create_listener_rules_tmp = [
    ...
    [
      ...
      [
        for alb_key, alb_values in try(port_values.load_balancer, {}) :
        [
          for rule_key, rule_values in try(alb_values.listener_rules, {}) :
          {
            "${instance_name}-${port_key}-${alb_key}-${rule_key}" = {
              "target_group_key" = "${instance_name}-${port_key}-${alb_key}"
              "name"             = "${local.common_name}-${instance_name}-${port_values.instance_port}"
              "port_key"         = port_key
      ...
      "${instance_name}" = [
        for port_key, port_values in try(instance_config.ports, {}) : [
          for alb_key, lb_config in try(port_values.load_balancer, {}) : {
            "custom_key"       = "${instance_name}-${port_key}-${alb_key}"
            "target_group_arn" = try(lb_config.target_group_attach, null) != null ? data.aws_lb_target_group.attach["${instance_name}-${port_key}-${alb_key}"].arn : aws_lb_target_group.this["${instance_name}-${port_key}-${alb_key}"].arn
            "instance_port"    = try(port_values["host_port"], port_values["instance_port"])
          }
        ]
      ]
    }
    if try(instance_config.ports, {}) != {}
  ]
...
locals {
  load_balancer_attachment = merge(flatten([
    for instance_name, instance_values in local.load_balancer_calculated : [
      for key, values in instance_values : {
        "${instance_name}-${key}" = merge(values, {
          instance_name = instance_name
        })
      }
    ]
...
resource "aws_lb_target_group" "this" {
  ...
  name                 = try(each.value.alb_values.target_group_custom_name, each.value.name)
  vpc_id               = data.aws_vpc.this[each.value.instance_name].id
  port                 = each.value.port_values.instance_port
  protocol             = try(each.value.alb_values.protocol, "HTTP")
  target_type          = try(each.value.alb_values.target_type, "instance")
  deregistration_delay = try(each.value.alb_values.deregistration_delay, 300)
  slow_start           = try(each.value.alb_values.slow_start, 0)

  health_check {
    path                = try(each.value.alb_values.health_check.path, "/")
    port                = try(each.value.alb_values.health_check.port, "traffic-port")
    protocol            = try(each.value.alb_values.health_check.protocol, each.value.alb_values.protocol, "HTTP")
    matcher             = try(each.value.alb_values.health_check.matcher, 200)
    interval            = try(each.value.alb_values.health_check.interval, 30)
    timeout             = try(each.value.alb_values.health_check.timeout, 5)
    healthy_threshold   = try(each.value.alb_values.health_check.healthy_threshold, 3)
    unhealthy_threshold = try(each.value.alb_values.health_check.unhealthy_threshold, 3)
  }
...
data "aws_lb_listener" "this" {
  ...
  port              = try(each.value.alb_values.alb_listener_port, 443)
}
...
resource "aws_lb_target_group_attachment" "this" {
  for_each = local.load_balancer_attachment

  target_group_arn = each.value.target_group_arn
  target_id        = module.ec2_instance[each.value.instance_name].id
  ...
  create_dns_records_tmp = [
    ...
    [
      ...
      [
        for alb_key, alb_values in try(port_values.load_balancer, {}) :
        [
          for dns_key, dns_values in try(alb_values.dns_records, {}) :
          {
            "${instance_name}-${port_key}-${alb_key}-${dns_key}" = {
              "target_group_key" = "${instance_name}-${port_key}-${alb_key}"
              "record_name"      = length(dns_key) > 0 ? dns_key : "${instance_name}-${port_key}-${alb_key}"
