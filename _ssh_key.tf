locals {
  ssh_key_parameters = {
    ...
    ec2_instance_key => {
      ...
      tags                  = merge(lookup(ec2_instance_config, "tags", local.default_common_tags), try(each.value.tags, var.ec2_instance_defaults, null), { Name = try(ec2_instance_config.key_name, "${local.common_name}-${ec2_instance_key}") })
